...
import StackDisplayCode from "./StackDisplayCode";
import { jsStackCode, cppStackCode } from "./StackDisplayCode";
import toast from "react-hot-toast";

export default function StackCode() {
  const [language, setLanguage] = useState("javascript");
  const [copied, setCopied] = useState(false);
  const jsCodeToCopy = jsStackCode;
  const cppCodeToCopy = cppStackCode;
  ...
  return (
    <div>
      <header className="bg-white w-200 ">
        ...
        <StackDisplayCode Language={language} />
      </header>
    </div>
  );
}
