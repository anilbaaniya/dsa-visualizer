...
import PostfixDisplayCode, {
  cppPostfixCode,
  jsPostfixCode,
} from "./PostfixDisplayCode";
import toast from "react-hot-toast";
export default function PostfixCode() {
  const [language, setLanguage] = useState("javascript");
  const [copied, setCopied] = useState(false);
  const jsCodeToCopy = jsPostfixCode;
  const cppCodeToCopy = cppPostfixCode;
  ...
  return (
    <div>
      <header className="bg-white w-200 ">
        ...
        <PostfixDisplayCode Language={language} />
      </header>
    </div>
  );
}
