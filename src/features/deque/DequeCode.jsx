...
import DequeCodeDisplay, {
  cppDequeCode,
  jsDequeCode,
} from "./DequeCodeDisplay";
import toast from "react-hot-toast";

export default function DequeCode() {
  const [language, setLanguage] = useState("javascript");
  const [copied, setCopied] = useState(false);
  const jsCodeToCopy = jsDequeCode;
  const cppCodeToCopy = cppDequeCode;
  ...
  return (
    <div>
      <header className="bg-white w-230 ">
        ...
        <DequeCodeDisplay Language={language} />
      </header>
    </div>
  );
}
