...
import CircularQueueCodeDisplay, {
  cppCircularQueueCode,
  jsCircularQueueCode,
} from "./CircularQueueCodeDisplay";
import toast from "react-hot-toast";

export default function CircularQueueCode() {
  const [language, setLanguage] = useState("javascript");
  const [copied, setCopied] = useState(false);
  const jsCodeToCopy = jsCircularQueueCode;
  const cppCodeToCopy = cppCircularQueueCode;
  ...
  return (
    <div>
      <header className="bg-white w-200 ">
        ...
        <CircularQueueCodeDisplay Language={language} />
      </header>
    </div>
  );
}
